...
async function testConcurrency(): Promise<void> {
  ...
  try {
    ...
    assert(
      getGpuConcurrency(ENDPOINT_ID) === 5,
      `Active slots: ${getGpuConcurrency(ENDPOINT_ID)} (expected 5)`,
    );
  ...
  assert(
    getGpuConcurrency(ENDPOINT_ID) === 0,
    `After release: ${getGpuConcurrency(ENDPOINT_ID)} active`,
  );
}
...
async function testExecutor(): Promise<void> {
  ...
  assert(
    result.gpuCostLight >= 0,
    `gpuCostLight: ${result.gpuCostLight.toFixed(6)}✦`,
  );
  assert(result.durationMs > 0, `durationMs: ${result.durationMs}ms`);
  assert(
    Array.isArray(result.logs),
    `logs is array: ${result.logs.length} lines`,
  );
...
async function testExecutorTimed(): Promise<void> {
  ...
  console.log(
    `  ${INFO} Expected cost for 2000ms on A40: ${
      expectedCost.toFixed(4)
    } Light`,
  );
  console.log(
    `  ${INFO} Actual cost from executor: ${
      result.gpuCostLight.toFixed(4)
    } Light`,
  );
  ...
  assert(
    result.gpuCostLight > 0,
    `gpuCostLight > 0: ${result.gpuCostLight.toFixed(6)}`,
  );
  ...
  assert(
    ...
    `Cost reasonable (actual: ${result.gpuCostLight.toFixed(4)}, expected: ~${
      expectedCost.toFixed(4)
    }, max: ${(expectedCost * 10).toFixed(4)})`,
  );
}
...
async function testExecutorFail(): Promise<void> {
  ...
  assert(
    ...
    `exitCode: ${result.exitCode} (expected exception)`,
  );
...
async function testCostBreakdown(): Promise<void> {
  ...
  const breakdown = computeGpuCallCost(
    fakeApp,
    "add",
    { a: 10, b: 20 },
    result,
  ...
  assert(
    breakdown.computeCostLight >= 0,
    `computeCost: ${breakdown.computeCostLight.toFixed(6)} Light`,
  );
  assert(
    breakdown.developerFeeLight === 0,
    `developerFee: ${breakdown.developerFeeLight} (no pricing config)`,
  );
  assert(
    breakdown.totalCostLight === breakdown.computeCostLight,
    "total = compute (no dev fee)",
  );
  assert(
    breakdown.ownerRevenueLight === 0,
    `ownerRevenue: ${breakdown.ownerRevenueLight} (no pricing)`,
  );

  console.log(
    `  ${INFO} Breakdown: compute=${
      breakdown.computeCostLight.toFixed(6)
    }, devFee=${breakdown.developerFeeLight}, total=${
      breakdown.totalCostLight.toFixed(6)
    }`,
  );
  ...
  const pricedBreakdown = computeGpuCallCost(
    pricedApp,
    "add",
    { a: 10, b: 20 },
    result,
  ...
  assert(
    pricedBreakdown.developerFeeLight === 10,
    `per_call fee: ${pricedBreakdown.developerFeeLight} Light`,
  );
  assert(
    pricedBreakdown.ownerRevenueLight === 8.5,
    `owner revenue: ${pricedBreakdown.ownerRevenueLight} Light (85% of 10)`,
  );
  assert(
  ...
  console.log(
    `  ${INFO} Per-call breakdown: compute=${
      pricedBreakdown.computeCostLight.toFixed(4)
    }, devFee=${pricedBreakdown.developerFeeLight}, total=${
      pricedBreakdown.totalCostLight.toFixed(4)
    }`,
  );
}
...
function testMaxCostEstimation(): void {
  ...
  assert(
    ...
    `Free app max cost: ${maxCost.toFixed(4)} Light (expected ${
  ...
  assert(
    ...
    `Per-call app max cost: ${pricedMax.toFixed(4)} Light (expected ${
      (expectedMax + 10).toFixed(4)
    })`,
  );
...
async function testSettlementSelfCall(): Promise<void> {
  ...
  assert(
    settlement.charged === false,
    `charged: ${settlement.charged} (self-call = free)`,
  );
  assert(
    settlement.chargedLight === 0,
    `chargedLight: ${settlement.chargedLight}`,
  );
  assert(
    settlement.failurePolicy === "full_charge",
    `failurePolicy: ${settlement.failurePolicy}`,
  );
...
async function testSettlementInfraError(): Promise<void> {
  ...
  assert(
    settlement.failurePolicy === "full_refund",
    `failurePolicy: ${settlement.failurePolicy}`,
  );
