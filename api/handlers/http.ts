...
export async function handleHttpEndpoint(request: Request, appId: string, path: string): Promise<Response> {
  ...
  try {
    ...
    // Rate limiting
    const rateLimit = (app as Record<string, unknown>).http_rate_limit as number || 1000;
    // TODO: Implement actual rate limiting check
    // For now, just log
    console.log(`[HTTP] Rate limit for app ${appId}: ${rateLimit} req/min`);
    ...
    // Log the request (for rate limiting and analytics)
    // TODO: Store in http_requests table for rate limiting
