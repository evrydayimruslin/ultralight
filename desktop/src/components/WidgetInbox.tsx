...
export default function WidgetInbox() {
  const { sources, data, loading, totalBadge, executeAction, refresh } = useWidgetInbox();
  ...
  if (loading && groups.length === 0) {
    return (
      <div className="text-center py-8">
        ...
        <p className="text-caption text-ul-text-muted mt-2">Loading widgets…</p>
      </div>
    );
  }

  if (groups.length === 0) {
    return null; // Don't show anything if no widgets have items
  }

  return (
    <div className="space-y-4">
      {groups.map(({ source, data: widgetData }) => (
        <div key={source.appId + ':' + source.widget.id}>
          {/* Source header */}
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              <span className="text-small font-medium text-ul-text">{source.widget.label}</span>
