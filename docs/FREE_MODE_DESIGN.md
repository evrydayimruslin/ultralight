...
- **✅ ENABLED FOR LAUNCH (2026-06-25):** `FREE_MODE = "1"` set in `api/wrangler.toml` `[vars]` (production) and `[env.staging.vars]` (staging, so the staging smoke exercises the gates before promotion). Migration `20260625000000` ships ahead of the API via the supabase-db CI in the promotion chain, so the `p_free_mode` arg is present before the flag flips. **Release note (gap §6.6): ✅ written** — see [docs/RELEASE_NOTES.md](RELEASE_NOTES.md) (Free Mode + the $0.50 → $0.25 inference-floor drop, dollars-only). **E2E smoke: ✅ added** — `scripts/smoke/free-mode-e2e-smoke.mjs` (+ `scripts/smoke/fixtures/free-mode-fixture/`) proves the gates on a live deployment.
